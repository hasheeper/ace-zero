...
(function (global) {
   ...
   *   short: "10.5金" / "0.3金"   (默认，UI 用)
   *   full:  "10.5金弗"           (叙事/日志用)
   *   chip:  "1×绯红 + 2×深蓝"   (筹码分解)
   * @param {boolean} [opts.alwaysGold=false] - 兼容旧参数；当前始终显示金单位
   * @returns {string}
   */
  ...
  /**
   * 彩色 HTML: 整数部分高亮，小数部分次高亮
   * 例: 980 → '<span style="...">9</span><span style="...">.8金</span>'
   * 例: 1000 → '<span style="...">10金</span>'
   * 例: 50 → '<span style="...">0</span><span style="...">.5金</span>'
   * @param {number} silver
  ...
  /**
   * 彩色 HTML (单一单位): 始终输出金弗小数
   * 用于按钮等空间有限的地方
   * 例: 1500 → '<span style="...">15金</span>'
   * 例: 50 → '<span style="...">0.5金</span>'
  ...
  function formatHtmlAmount(silver) {
    ...
    var prefix = negative ? '-' : '';
    return prefix + '<span style="' + GOLD_CSS + '">' + _formatGoldStringFromSilver(val) + '金</span>';
  }
