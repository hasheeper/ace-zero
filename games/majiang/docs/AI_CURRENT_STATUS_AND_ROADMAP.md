...
- H15 v1/v2 call-suppression overlays were rejected by arena scout. H15c replaces broad call blocking with closed-route value rebalance, then splits the hard family into four benchmark personalities: `hard-aggressive` (legacy pure speed), `hard-defensive` (legacy tuned/formal baseline), `hard-balanced` (mild closed-route scoring), and `hard-heavy` (stronger closed-riichi route value scoring). Formal `hard` remains unchanged.
- H16 records the first four-personality arena scout: `hard-heavy` is currently strongest, `hard-aggressive` has the intended speed but high fourth rate, `hard-defensive` controls deal-in but needs counterattack, and stable `hard-balanced` is route-inactive. The stable four personalities are now frozen checkpoints. Dev entries `hard-aggressive-dev`, `hard-defensive-dev`, `hard-balanced-dev`, and `hard-heavy-dev` are benchmark-visible; only `hard-balanced-dev` currently changes behavior, using a lightweight balanced route state layer on top of the shared closed-route scorer.
