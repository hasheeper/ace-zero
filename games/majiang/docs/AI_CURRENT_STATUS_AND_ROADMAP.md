...
- H15 v1/v2 call-suppression overlays were rejected by arena scout. H15c replaces broad call blocking with closed-route value rebalance, then splits the hard family into four benchmark personalities: `hard-aggressive` (legacy pure speed), `hard-defensive` (legacy tuned/formal baseline), `hard-balanced` (now promoted route-state balanced), and `hard-heavy` (stronger closed-riichi route value scoring). Formal `hard` remains unchanged.
- H16 records the first four-personality arena scout: `hard-heavy` is currently strongest, `hard-aggressive` has the intended speed but high fourth rate, `hard-defensive` controls deal-in but needs counterattack, and the original stable `hard-balanced` was route-inactive. The route-inactive balanced checkpoint is retired as a bug version. Dev entries `hard-aggressive-dev`, `hard-defensive-dev`, and `hard-balanced-dev` remain benchmark-visible.
- H17 starts `hard-defensive-dev`: stable `hard-defensive` remains frozen, while the dev variant adds scored threat review, rank-aware push/fold biasing, and compact deal-in attribution. H17b removed the attempted closed-route/riichi alignment after arena showed it created a heavy-like personality with higher deal-in. Arena summaries expose `defensiveState`, `threatReason`, and `dealInAttribution` for the dev defense path so the next scouts can judge whether lower deal-in and lower fourth-rate convert to rank.
- H18 repositions balanced/heavy without renaming stable variants. The `hard-balanced-candidate` clone and `hard-heavy-dev` route-weight experiment were retired after scout evidence showed they added no useful independent signal. The former `hard-balanced-dev` route-state profile has been promoted into stable `hard-balanced`; `hard-balanced-dev` now inherits stable balanced until the next explicit experiment.
...
| `hard` | H9 four-player Hard v1 formally delivered, H10/H11 Mortal alignment tracked with severity, H12 P0/P1/R0/P2/P3/P4/P5 delivered for diagnostics, no-pressure shape cleanup, safer evaluator mount points, low-danger defense tie-breaks, narrow thin tanki riichi acceptance, expanded same-xiangting shape review, and fixed-state tile-choice attribution; P6/P7 strategy patches retired after regression/overfitting evidence; H13-H13g evaluate a Mortal q-value ranker offline without changing gameplay; H14 gates experimental overlays with repair pools, replay, Mortal scout, and arena scout; H15 call-suppression overlays rejected, and H15c starts a safer closed-route value scoring overlay in `hard-experimental` only | 60-68% |
