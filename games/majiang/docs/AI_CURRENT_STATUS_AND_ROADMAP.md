...
- H14 keeps `hard-tuned` as the formal baseline and moves experimental work behind gates. P3 defense overlay was rejected by arena scout, P4 same-xiangting rerank had no replay-eligible real rows, and P5 adds an experimental ledger plus compact route diagnostics so remaining disagreements are reviewed as route/dora/five/tenpai issues before any new overlay.
