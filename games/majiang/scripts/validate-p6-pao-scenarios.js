...
function runScenario(configPath) {
  try {
    ...
    return {
      configPath,
      snapshot: { phase: 'error', roundResult: null },
...
function main() {
  ...
  const defaultConfigs = [
    'game-config.p6-daisangen-pao.json',
    'game-config.p6-daisuushi-pao.json'
  ...
  configPaths.forEach((configPath) => {
    ...
    if (result.errors.length) {
      ...
      console.error(`  phase=${result.snapshot.phase}`);
      console.error(`  roundResult=${JSON.stringify(result.snapshot.roundResult)}`);
      return;
    }

    console.log(`[PASS] ${name}`);
    console.log(`  phase=${result.snapshot.phase}`);
