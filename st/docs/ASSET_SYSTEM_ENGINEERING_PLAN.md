...
```text
ACT Asset 节点 / 外部结算
  -> 获得 Deck Points (`world.assetDeck.asset_count`)
  -> 打开 Asset 结算面板
  -> 扩容或抽卡
  -> 卡牌进入 active deck
  -> MVU 持久化
...
- AssetDeck 点数对玩家展示为 `DECK PTS / 卡组点`，代码真相源仍是 `world.assetDeck.asset_count`。
...
- `DECK POINTS : 3` 绑定 `world.assetDeck.asset_count`，不要读取剧情 funds/assets，也不要读取 `hero.assets`。
...
- Overview 顶部 `DECK PTS` 读取 `world.assetDeck.asset_count`，不再把 AssetDeck 点数混到 `hero.assets` 剧情资产。
...
- Asset 节点可以给 Deck Points (`asset_count`)，或打开 AssetDeck 结算面板。
...
```text
6.1 Asset 节点 grant_asset：只给 Deck Points (`asset_count`)，不开抽卡面板。已完成。
...
- 顶部 `DECK PTS` 资源值改读 `summary.points`。
- Planner 的 Asset Deck 页、Contract Extract 页、ACT Asset settlement 面板改读 `summary.slots / summary.activeCards / summary.pending / summary.points`。
